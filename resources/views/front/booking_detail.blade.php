<x-front-layout>
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8" x-data="{
    ...
    default => ucfirst($booking->status)
                ...
                <a href="{{ route('bookings.invoice', $booking) }}" target="_blank"
                    class="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 text-xs sm:text-sm font-medium rounded-lg border border-sky-300 text-sky-700 bg-white hover:bg-sky-50 transition">
                    🖨️ Cetak Invoice
                </a>

                @if(in_array($booking->status, ['disetujui', 'pending', 'menunggu pembayaran']) && $booking->canCancel())
                    <button @click="showCancelModal = true"
                        class="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 text-xs sm:text-sm font-medium rounded-lg border border-red-300 text-red-700 bg-white hover:bg-red-50 transition">
                        ✕ Batalkan
                    </button>
                @endif
                ...
                @if($booking->canExtend())
                    <a href="{{ route('bookings.extend', $booking) }}"
                        class="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 text-xs sm:text-sm font-medium rounded-lg border border-emerald-300 text-emerald-700 bg-white hover:bg-emerald-50 transition">
                        🔄 Perpanjang Sewa
                    </a>
                @endif
            </div>
        ...
        <!-- Status Timeline -->
        <div class="bg-white shadow-sm rounded-xl p-4 sm:p-6 mb-6 border border-slate-100">
            <h3 class="text-sm sm:text-base font-semibold text-slate-700 mb-4">Status Pesanan</h3>
            <div class="flex flex-col sm:flex-row items-center gap-2 sm:gap-4 text-xs sm:text-sm">
                ...
                @foreach($statuses as $status => $label)
                    ...
                    <div class="flex flex-col items-center">
                        ...
                        <span class="text-xs sm:text-sm text-slate-600 mt-1">{{ $label }}</span>
                    </div>
                    @if(!$loop->last)
                        <div class="hidden sm:block flex-1 h-0.5 mx-1 {{ $isNextCompleted ? 'bg-sky-600' : 'bg-slate-200' }}"></div>
                    @endif
                @endforeach
            </div>
        </div>
        ...
        <!-- Informasi Mobil -->
        <div class="bg-white shadow-sm rounded-lg sm:rounded-xl mb-6 border border-slate-100 overflow-hidden">
            <div class="px-4 py-4 sm:px-6 sm:py-5 bg-slate-50 border-b border-slate-200">
                <h3 class="text-base sm:text-lg font-bold text-gray-900">🚗 Informasi Mobil</h3>
            </div>
            <div class="border-t border-gray-200">
                <dl class="divide-y divide-gray-200">
                    <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                        <dt class="text-xs sm:text-sm font-medium text-gray-500">Tipe Mobil</dt>
                        <dd class="text-sm sm:text-base text-gray-900 font-semibold sm:col-span-2">
                            {{ $booking->car->brand }} {{ $booking->car->name }}</dd>
                    </div>
                    @if($booking->car_unit_id)
                        <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                            <dt class="text-xs sm:text-sm font-medium text-gray-500">Plat Nomor & Warna</dt>
                            <dd class="text-sm sm:text-base text-gray-900 sm:col-span-2">
                                <span
                                    class="inline-block bg-sky-100 text-sky-800 px-2.5 py-1 rounded-lg font-bold mr-2 text-xs sm:text-sm">{{ $booking->carUnit->license_plate }}</span>
                                <span class="text-gray-600 text-xs sm:text-sm">({{ $booking->carUnit->color }})</span>
                            </dd>
                        </div>
                    @else
                        <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                            <dt class="text-xs sm:text-sm font-medium text-gray-500">Plat Nomor & Warna</dt>
                            <dd class="text-xs sm:text-sm text-gray-500 italic sm:col-span-2">⏳ Akan ditambahkan setelah admin approve pesanan.</dd>
                        </div>
                    @endif

                    <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                        <dt class="text-xs sm:text-sm font-medium text-gray-500">Spesifikasi Kendaraan</dt>
                        <dd class="text-sm sm:text-base text-gray-900 sm:col-span-2">
                            <div class="grid grid-cols-2 gap-2 sm:gap-4 bg-slate-50 p-3 sm:p-4 rounded-lg border border-slate-200">
                                <div>
                                    <span
                    ...
                    <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                        <dt class="text-xs sm:text-sm font-medium text-gray-500">Metode Pembayaran</dt>
                        <dd class="text-xs sm:text-sm text-gray-900 uppercase font-bold text-sky-600 sm:col-span-2">
                    ...
                    @if($booking->payment_method === 'transfer_manual')
                        <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                            <dt class="text-xs sm:text-sm font-medium text-gray-500">Detail Transfer</dt>
                            <dd class="text-sm sm:text-base text-gray-900 sm:col-span-2">
                                @php
                                    $bankDetails = [
                        ...
                        @if($booking->payment_method === 'transfer_manual' && ($booking->proof_image || $booking->proof_link))
                            <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                                <dt class="text-xs sm:text-sm font-medium text-gray-500">Bukti Transfer</dt>
                                <dd class="text-sm sm:text-base text-gray-900 sm:col-span-2">
                                    <div class="space-y-3">
        ...
        <div class="bg-white shadow-sm rounded-lg sm:rounded-xl mb-6 border border-slate-100 overflow-hidden">
            <div class="px-4 py-4 sm:px-6 sm:py-5 bg-slate-50 border-b border-slate-200">
                <h3 class="text-base sm:text-lg font-bold text-gray-900">📅 Informasi Sewa</h3>
            </div>
            <div class="border-t border-gray-200">
                <dl class="divide-y divide-gray-200">
                    <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                        <dt class="text-xs sm:text-sm font-medium text-gray-500">Tanggal & Durasi</dt>
                        <dd class="text-sm sm:text-base text-gray-900 sm:col-span-2">
                    ...
                    <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                        <dt class="text-xs sm:text-sm font-medium text-gray-500">Mode Pengambilan</dt>
                        <dd class="text-sm sm:text-base text-gray-900 sm:col-span-2">
                    ...
                    <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                        <dt class="text-xs sm:text-sm font-medium text-gray-500">Opsi Layanan</dt>
                        <dd class="text-sm sm:text-base text-gray-900 sm:col-span-2">
                            @if($booking->with_driver)
                                ...
                                @if($booking->driver)
                                    <div class="mt-2 bg-green-50 border border-green-200 rounded-lg p-2">
                                        <div class="flex items-center gap-1.5">
                                            <span class="text-xs font-semibold text-green-900">👨‍✈️ {{ $booking->driver->name }}</span>
                                        </div>
                                ...
                                @else
                                    <p class="text-xs text-slate-500 italic mt-1">⏳ Sopir akan ditentukan setelah admin menyetujui pesanan.</p>
                                @endif
                            @else
                                <span
        ...
        @if($booking->with_driver && $booking->driver)
            <div class="bg-white shadow-sm rounded-lg sm:rounded-xl mb-6 border border-slate-100 overflow-hidden">
                <div class="px-4 py-4 sm:px-6 sm:py-5 bg-slate-50 border-b border-slate-200">
                    <h3 class="text-base sm:text-lg font-bold text-gray-900">👤 Informasi Sopir</h3>
                </div>
                <div class="border-t border-gray-200">
                    <dl class="divide-y divide-gray-200">
                        <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                            <dt class="text-xs sm:text-sm font-medium text-gray-500">Nama Sopir</dt>
                            <dd class="text-sm sm:text-base text-gray-900 font-semibold sm:col-span-2">
                                {{ $booking->driver->name }}</dd>
                        </div>
                        <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                            <dt class="text-xs sm:text-sm font-medium text-gray-500">Nomor Telepon</dt>
                            <dd class="text-sm sm:text-base text-gray-900 sm:col-span-2">
                                <a href="tel:{{ $booking->driver->phone }}"
                        ...
                        <div class="px-4 py-4 sm:px-6 sm:py-5 grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4">
                            <dt class="text-xs sm:text-sm font-medium text-gray-500">Status Sopir</dt>
                            <dd class="text-sm sm:text-base text-gray-900 sm:col-span-2">
        ...
        <div class="bg-white shadow-sm rounded-lg sm:rounded-xl mb-6 border border-slate-100 overflow-hidden">
            <div class="px-4 py-4 sm:px-6 sm:py-5 bg-rose-50 border-b border-rose-100 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                <h3 class="text-base sm:text-lg font-bold text-rose-900">🚨 Pusat Bantuan & Laporan</h3>
                <span class="text-xs font-semibold text-rose-700 bg-rose-100 px-2 py-1 rounded-full w-fit">Layanan 24 Jam</span>
            </div>
            <div class="px-4 py-4 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                <!-- Komplain Pelayanan -->
                <div class="bg-amber-50 border border-amber-200 rounded-lg sm:rounded-xl p-3 sm:p-4 flex flex-col sm:flex-row gap-3 sm:gap-4">
                    <div class="text-2xl sm:text-3xl shrink-0">🗣️</div>
                    <div class="flex-1">
                        <h4 class="font-bold text-amber-900 mb-1 text-sm sm:text-base">Komplain Pelayanan</h4>
                        <p class="text-xs sm:text-sm text-amber-800 mb-3">Sopir tidak ramah, mobil kurang bersih, fasilitas kurang, atau kendala pelayanan lainnya.</p>
                        <a href="https://wa.me/6281234567890?text=Halo%20Admin%20AutoRent,%20saya%20ingin%20menyampaikan%20komplain%20pelayanan%20terkait%20pesanan%20%23{{ str_pad($booking->id, 5, '0', STR_PAD_LEFT) }}.%0A%0AKendala%20yang%20dialami:%20..."
                            target="_blank"
                            class="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 text-xs font-bold rounded-lg bg-amber-500 text-white hover:bg-amber-600 transition shadow-sm">
                ...
                <!-- Darurat / Kecelakaan -->
                <div class="bg-rose-50 border border-rose-200 rounded-lg sm:rounded-xl p-3 sm:p-4 flex flex-col sm:flex-row gap-3 sm:gap-4">
                    <div class="text-2xl sm:text-3xl shrink-0">⚠️</div>
                    <div class="flex-1">
                        <h4 class="font-bold text-rose-900 mb-1 text-sm sm:text-base">Darurat & Kecelakaan</h4>
                        <p class="text-xs sm:text-sm text-rose-800 mb-3">Mobil mogok di jalan, terjadi kecelakaan, atau kondisi darurat bencana alam.</p>
                        <a href="https://wa.me/6281234567890?text=URGENT%20AutoRent!%20Saya%20mengalami%20keadaan%20darurat%20pada%20pesanan%20%23{{ str_pad($booking->id, 5, '0', STR_PAD_LEFT) }}.%0A%0AJenis%20Darurat%20(Kecelakaan/Mogok/Bencana):%20...%0ALokasi%20Saat%20Ini:%20...%0AKondisi%20Penumpang:%20..."
                            target="_blank"
                            class="inline-flex items-center justify-center w-full sm:w-auto px-3 py-2 text-xs font-bold rounded-lg bg-rose-600 text-white hover:bg-rose-700 transition shadow-sm">
        ...
        <div class="bg-gradient-to-r from-blue-50 to-indigo-50 shadow-sm rounded-lg sm:rounded-xl p-4 sm:p-6 mb-6 border border-blue-100">
            <h3 class="text-base sm:text-lg font-bold text-blue-900 mb-4">📋 Kebijakan Pembatalan & Refund</h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
                ...
                <div class="bg-white rounded-lg p-3 sm:p-4 border border-blue-100">
                    <div class="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-3">
                        ...
                        <div class="flex-1">
                            <h4 class="font-semibold text-slate-900 mb-1 text-sm sm:text-base">Deadline Pembatalan</h4>
                            @php
                ...
                <div class="bg-white rounded-lg p-3 sm:p-4 border border-blue-100">
                    <div class="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-3">
                        <div class="text-xl sm:text-2xl shrink-0">💰</div>
                        <div class="flex-1">
                            <h4 class="font-semibold text-slate-900 mb-2 text-sm sm:text-base">Estimasi Refund</h4>
            ...
            <div class="mt-3 sm:mt-4 bg-white rounded-lg p-3 sm:p-4 border border-blue-100">
        ...
        @if($booking->status === 'dibatalkan' || $booking->status === 'pending_review')
                <div
                    class="bg-white shadow-sm rounded-lg sm:rounded-xl p-4 sm:p-6 mb-6 border-l-4 {{ $booking->status === 'dibatalkan' ? 'border-l-red-500' : 'border-l-orange-500' }} border border-slate-100">
                    <h3 class="text-base sm:text-lg font-bold text-slate-900 mb-4">📊 Status Pembatalan & Refund</h3>
        ...
        @if($booking->extensions && $booking->extensions->isNotEmpty())
            <div class="bg-white shadow-sm border border-slate-100 rounded-lg sm:rounded-xl mb-6 overflow-hidden">
                <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100 bg-sky-50">
                    <h3 class="text-sm sm:text-base font-bold text-sky-900">🔄 Riwayat Perpanjangan Sewa</h3>
                </div>
                <div class="divide-y divide-slate-100">
                    @foreach($booking->extensions as $ext)
                        <div class="px-4 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                            <div class="flex-1">
                                <p class="text-xs sm:text-sm font-semibold text-slate-900">Tambah {{ $ext->extra_days }} Hari</p>
                                <p class="text-xs text-slate-500 mt-1">Sampai dengan {{ $ext->new_end_date->format('d M Y') }}</p>
                                @if($ext->reason)
                                    <p class="text-xs text-slate-500 mt-1 italic">Alasan: {{ $ext->reason }}</p>
                                @endif
                            </div>
                            <div class="text-left sm:text-right">
                                <p class="text-xs sm:text-sm font-bold text-slate-900">Rp {{ number_format($ext->extra_price, 0, ',', '.') }}</p>
                                <p class="text-xs mt-1 font-semibold {{ $ext->status === 'approved' ? 'text-emerald-600' : ($ext->status === 'rejected' ? 'text-red-600' : 'text-amber-500') }}">
                                    {{ $ext->status_label }}
                                </p>
                            </div>
                        </div>
        ...
        @if($booking->addons->isNotEmpty() || $booking->discount_amount > 0 || $booking->deposit_amount > 0)
            <div class="bg-white shadow-sm border border-slate-100 rounded-lg sm:rounded-xl mb-6 overflow-hidden">
                <div class="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-100 bg-slate-50">
                    <h3 class="text-sm sm:text-base font-bold text-slate-900">💰 Rincian Tambahan & Potongan</h3>
                </div>
                <div class="px-4 sm:px-6 py-3 sm:py-4 space-y-3 text-xs sm:text-sm">
        ...
        <!-- Total Tagihan -->
        <div class="bg-gradient-to-r from-sky-500 to-sky-600 shadow-lg overflow-hidden rounded-lg sm:rounded-xl mb-6 text-white">
            <div class="px-4 sm:px-6 py-4 sm:py-6 border-b border-sky-400">
                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-4">
                    <h3 class="text-base sm:text-lg font-bold opacity-90">💳 Total Tagihan Sewa</h3>
                    <span class="text-xl sm:text-2xl font-bold">Rp {{ number_format($booking->total_price, 0, ',', '.') }}</span>
                </div>

                @if($booking->dp_amount > 0)
                    <div class="flex justify-between items-center mb-2 text-sky-100 text-xs sm:text-sm">
                        <span>Sudah Dibayar (DP)</span>
                        <span class="font-semibold">- Rp {{ number_format($booking->dp_amount, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-4 border-t border-sky-400 mt-2 gap-3">
                        <div>
                            <span class="text-base sm:text-lg font-bold">Sisa Pembayaran</span>
                            <p class="text-xs text-sky-100 opacity-80 mt-1">Harus dilunasi sebelum pengambilan unit</p>
                        </div>
                        <span class="text-2xl sm:text-4xl font-extrabold text-yellow-300">Rp {{ number_format(max(0, $booking->total_price - $booking->dp_amount), 0, ',', '.') }}</span>
                    </div>
                @else
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center pt-4 border-t border-sky-400 mt-2 gap-3">
                        <div>
                            <span class="text-base sm:text-lg font-bold">Yang Harus Dibayar</span>
                            <p class="text-xs text-sky-100 opacity-80 mt-1">Harga akhir yang harus dilunasi</p>
                        </div>
                        <span class="text-2xl sm:text-4xl font-extrabold text-yellow-300">Rp {{ number_format($booking->total_price, 0, ',', '.') }}</span>
                    </div>
                @endif
            </div>

            @if($booking->status === 'menunggu pembayaran')
                <div class="border-t border-sky-400 px-4 sm:px-6 py-4 sm:py-5 text-center bg-sky-500 bg-opacity-50">
                    <p class="text-sky-100 mb-4 font-medium text-xs sm:text-sm">Silakan selesaikan pembayaran untuk mengkonfirmasi pesanan Anda.</p>
                    <a href="{{ route('payment.show', $booking) }}"
                        class="inline-flex items-center justify-center w-full sm:w-auto px-4 sm:px-6 py-2 sm:py-3 border border-white text-xs sm:text-base font-bold rounded-lg sm:rounded-xl shadow-sm text-sky-600 bg-white hover:bg-slate-100 transition-colors">
                        💳 Bayar Sekarang →
                    </a>
                </div>
            @elseif($booking->status === 'pending')
                <div class="border-t border-sky-400 px-4 sm:px-6 py-4 sm:py-5 text-center bg-sky-500 bg-opacity-50">
                    <p class="text-sky-100 font-medium text-xs sm:text-sm">⏳ Pesanan Anda sedang direview oleh Admin. Mohon tunggu.</p>
                </div>
            @endif
        </div>
        ...
        @if($booking->status === 'selesai')
            <div class="bg-white rounded-lg sm:rounded-2xl shadow-sm border border-slate-100 p-4 sm:p-6 mb-6">
                @if($booking->review)
                    <h3 class="text-base sm:text-lg font-bold text-slate-900 mb-4">⭐ Ulasan & Kuesioner Anda</h3>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6">
                        <!-- Kuesioner Cabang -->
                        <div class="bg-slate-50 p-3 sm:p-4 rounded-lg sm:rounded-xl border border-slate-100">
                            <h4 class="font-bold text-slate-800 mb-3 text-sm sm:text-base">🏢 Layanan Cabang</h4>
                        ...
                        <!-- Kuesioner Kendaraan -->
                        <div class="bg-slate-50 p-3 sm:p-4 rounded-lg sm:rounded-xl border border-slate-100">
                            <h4 class="font-bold text-slate-800 mb-3 text-sm sm:text-base">🚗 Kondisi Kendaraan</h4>
                            <div class="space-y-2 text-xs sm:text-sm">
                                <div class="flex justify-between items-center">
                ...
                @else
                        <h3 class="text-base sm:text-lg font-bold text-slate-900 mb-1">📋 Kuesioner & Ulasan Pengalaman Anda</h3>
                        <p class="text-slate-500 text-xs sm:text-sm mb-5">Feedback Anda sangat berarti untuk meningkatkan kualitas layanan AutoRent ke depannya.</p>

                        @if(session('success'))
                            <div class="mb-4 bg-emerald-50 border border-emerald-200 rounded-lg sm:rounded-xl px-4 py-3 text-emerald-800 text-xs sm:text-sm font-medium">
                                {{ session('success') }}</div>
                        @endif
                        ...
                        <form action="{{ route('bookings.review', $booking) }}" method="POST" x-data="{
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6">
                                <!-- Kuesioner Cabang -->
                                <div class="bg-slate-50 p-4 sm:p-5 rounded-lg sm:rounded-xl border border-slate-100">
                                    <h4 class="font-bold text-slate-800 mb-4 flex items-center gap-2 text-sm sm:text-base">🏢 Layanan Cabang</h4>

                                    <div class="mb-4">
                                        <label class="block text-xs sm:text-sm font-semibold text-slate-700 mb-1">Kualitas Pelayanan</label>
                                    ...
                                    <div>
                                        <label class="block text-xs sm:text-sm font-semibold text-slate-700 mb-1">Keramahan Staf</label>
                                        <div class="flex gap-1 sm:gap-2">
                                            @for($star = 1; $star <= 5; $star++)
                                ...
                                <!-- Kuesioner Kendaraan -->
                                <div class="bg-slate-50 p-4 sm:p-5 rounded-lg sm:rounded-xl border border-slate-100">
                                    <h4 class="font-bold text-slate-800 mb-4 flex items-center gap-2 text-sm sm:text-base">🚗 Kendaraan</h4>

                                    <div class="mb-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                                        <label class="block text-xs sm:text-sm font-semibold text-slate-700">Kebersihan</label>
                                        <div class="flex gap-1 sm:gap-2">
                                            @for($star = 1; $star <= 5; $star++)
                                    ...
                                    <div class="mb-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                                        <label class="block text-xs sm:text-sm font-semibold text-slate-700">Kenyamanan</label>
                                        <div class="flex gap-1 sm:gap-2">
                                            @for($star = 1; $star <= 5; $star++)
                                    ...
                                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
                                        <label class="block text-xs sm:text-sm font-semibold text-slate-700">Kondisi Mesin</label>
                                        <div class="flex gap-1 sm:gap-2">
                                            @for($star = 1; $star <= 5; $star++)
                            ...
                            <div class="border-t border-slate-200 pt-4 sm:pt-6">
                                <h4 class="font-bold text-slate-800 mb-4 flex items-center gap-2 text-sm sm:text-base">⭐ Penilaian Umum & Komentar</h4>
                                <div class="mb-4">
                                    <label class="block text-xs sm:text-sm font-semibold text-slate-700 mb-1">Rating Keseluruhan <span class="text-red-500">*</span></label>
                                    <div class="flex gap-1 sm:gap-2">
                                        @for($star = 1; $star <= 5; $star++)
                                ...
                                <div class="mb-4">
                                    <label class="block text-xs sm:text-sm font-semibold text-slate-700 mb-1">Komentar / Ulasan</label>
                                    <textarea name="comment" rows="3"
                                        placeholder="Ceritakan pengalaman Anda secara keseluruhan menyewa di AutoRent..."
                                        class="w-full rounded-lg sm:rounded-xl border-slate-200 bg-slate-50 focus:bg-white focus:ring-2 focus:ring-sky-500 transition text-xs sm:text-sm py-2 sm:py-3 px-3 sm:px-4"></textarea>
                                    @error('comment') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                                </div>
                            </div>

                            <button type="submit" :disabled="rating === 0"
                                class="px-4 sm:px-6 py-2 sm:py-3 bg-sky-600 text-white font-bold rounded-lg sm:rounded-xl hover:bg-sky-700 transition disabled:opacity-50 disabled:cursor-not-allowed w-full sm:w-auto text-xs sm:text-sm">
    ...
    <div x-show="showCancelModal" x-transition
        ...
        <div x-show="showCancelModal" x-transition
            class="bg-white rounded-lg sm:rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto"
            style="display: none;">
            <!-- Header -->
            <div
                class="sticky top-0 bg-gradient-to-r from-red-500 to-red-600 text-white px-4 sm:px-6 py-4 sm:py-6 border-b border-red-400 flex justify-between items-center">
                <h2 class="text-lg sm:text-xl font-bold">Batalkan Pesanan</h2>
                <button @click="showCancelModal = false"
                    class="text-xl sm:text-2xl leading-none opacity-70 hover:opacity-100">&times;</button>
            </div>

            <form action="{{ route('bookings.cancel', $booking) }}" method="POST" class="p-4 sm:p-6 space-y-4 sm:space-y-5">
                @csrf
                @method('PUT')

                <!-- Cancel Category Selection -->
                <div>
                    <label class="block text-xs sm:text-sm font-bold text-slate-900 mb-2">📌 Kategori Pembatalan <span class="text-red-500">*</span></label>
                    <select name="cancel_category" x-on:change="updateRefundEstimate()"
                        class="w-full px-3 sm:px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent text-xs sm:text-sm"
                        required>
                ...
                <div x-show="document.querySelector('[name=cancel_category]')?.value === 'damage'"
                    class="border-l-4 border-orange-400 bg-orange-50 p-3 sm:p-4 rounded-lg space-y-3" style="display: none;">
                    <h4 class="font-semibold text-orange-900 text-sm sm:text-base">💔 Informasi Kerusakan</h4>
                    ...
                    <div>
                        <label class="flex items-center gap-2 cursor-pointer">
                            ...
                            <span class="text-xs sm:text-sm font-medium text-slate-700">Kerusakan disebabkan oleh customer (saya)</span>
                        </label>
                    </div>
                    ...
                    <div>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="insurance_claimed"
                                class="w-4 h-4 border-slate-300 rounded focus:ring-2 focus:ring-red-500">
                            <span class="text-xs sm:text-sm font-medium text-slate-700">Asuransi sudah diklaim</span>
                        </label>
                    </div>

                    <div>
                        <label class="block text-xs sm:text-sm font-medium text-slate-700 mb-1">Deskripsi Kerusakan</label>
                        <textarea name="damage_description" rows="3" placeholder="Jelaskan kerusakan yang terjadi..."
                            class="w-full px-3 sm:px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent text-xs sm:text-sm"></textarea>
                    </div>
                </div>

                <!-- Reason -->
                <div>
                    <label class="block text-xs sm:text-sm font-bold text-slate-900 mb-2">💬 Alasan Pembatalan <span class="text-red-500">*</span></label>
                    <textarea name="cancelled_reason" rows="3" placeholder="Jelaskan alasan Anda membatalkan pesanan..."
                        class="w-full px-3 sm:px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent text-xs sm:text-sm"
                        required></textarea>
                    @error('cancelled_reason') <p class="text-red-500 text-xs sm:text-sm mt-1">{{ $message }}</p> @enderror
                </div>

                <!-- Refund Method -->
                <div>
                    <label class="block text-xs sm:text-sm font-bold text-slate-900 mb-2">💰 Metode Pengembalian Dana <span class="text-red-500">*</span></label>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-3">
                        <label
                            class="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 border border-slate-300 rounded-lg cursor-pointer hover:bg-blue-50 transition">
                            <input type="radio" name="refund_method" value="bank_transfer"
                                class="w-4 h-4 border-slate-300" checked>
                            <div>
                                <p class="font-semibold text-slate-900 text-xs sm:text-sm">🏦 Transfer Bank</p>
                                <p class="text-xs text-slate-600">1-3 hari kerja</p>
                            </div>
                        </label>
                        <label
                            class="flex items-center gap-2 sm:gap-3 p-2 sm:p-3 border border-slate-300 rounded-lg cursor-pointer hover:bg-green-50 transition">
                            <input type="radio" name="refund_method" value="wallet_credit"
                                class="w-4 h-4 border-slate-300">
                            <div>
                                <p class="font-semibold text-slate-900 text-xs sm:text-sm">💳 Wallet Credit</p>
                ...
                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg p-3 sm:p-4">
                    <h4 class="font-bold text-slate-900 mb-3 text-sm sm:text-base">📊 Estimasi Refund</h4>
                    <div class="space-y-2 text-xs sm:text-sm">
                ...
                <!-- Buttons -->
                <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-3 sm:pt-4 border-t border-slate-200">
                    <button type="button" @click="showCancelModal = false"
                        class="flex-1 px-3 sm:px-4 py-2 sm:py-3 border border-slate-300 text-slate-700 font-bold rounded-lg hover:bg-slate-50 transition text-xs sm:text-sm">
                        Batal
                    </button>
                    <button type="submit"
                        class="flex-1 px-3 sm:px-4 py-2 sm:py-3 bg-red-600 text-white font-bold rounded-lg hover:bg-red-700 transition text-xs sm:text-sm">
                        ✓ Batalkan Pesanan
                    </button>
                </div>
            </form>
