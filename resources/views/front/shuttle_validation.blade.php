<x-front-layout>
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div class="text-center mb-8">
            ...
            <p class="mt-2 text-lg text-gray-600">Dokumen tiket Shuttle yang Anda pindai terdaftar sah di sistem AutoRent.</p>
        </div>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-2xl border border-gray-200">
            <div class="px-4 py-5 sm:px-6 bg-slate-50 border-b border-gray-200 flex justify-between items-center">
                <h3 class="text-lg leading-6 font-bold text-gray-900">Informasi Tiket Shuttle</h3>
            ...
            <div class="border-t border-gray-200 px-4 py-5 sm:p-0">
                <dl class="sm:divide-y sm:divide-gray-200">
                    ...
                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 bg-slate-50">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 font-semibold">{{ $booking->user->name ?? 'Tamu' }}</dd>
                    </div>
                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Rute Perjalanan</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2 font-bold">{{ $booking->route->origin_city ?? '-' }} ➔ {{ $booking->route->destination_city ?? '-' }}</dd>
                    </div>
                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 bg-slate-50">
                        <dt class="text-sm font-medium text-gray-500">Jadwal Keberangkatan</dt>
                        <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                            {{ \Carbon\Carbon::parse($booking->travel_date)->format('d M Y') }} - {{ \Carbon\Carbon::parse($booking->route->departure_time)->format('H:i') }} WIB
                        </dd>
                    </div>
                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                        <dt class="text-sm font-medium text-gray-500">Nomor Kursi</dt>
