<x-front-layout>
            ...
            <div class="bg-white rounded-3xl shadow-xl shadow-slate-200/50 overflow-hidden border border-slate-100">
                <div class="p-8 lg:p-10">
                    ...
                    <div class="flex flex-col md:flex-row items-center gap-6 pb-8 border-b border-slate-100">
                        <div class="w-full md:w-48 h-32 bg-sky-100 rounded-xl flex items-center justify-center text-sky-500">
                        ...
                        <div class="flex-1 w-full text-center md:text-left">
                            <span class="inline-block px-3 py-1 rounded-lg text-xs font-bold bg-sky-100 text-sky-700 uppercase tracking-wider mb-2">Order #{{ $booking->booking_code }}</span>
                            <h2 class="text-2xl font-bold text-slate-900">Travel Shuttle: {{ $booking->route->origin_city }} ➔ {{ $booking->route->destination_city }}</h2>
                            <p class="text-slate-500 mt-1">Status Pembayaran: <span class="font-semibold text-amber-500">{{ strtoupper($booking->payment_status) }}</span></p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 py-8 border-b border-slate-100">
                        <div>
                            <h3 class="text-sm font-bold text-slate-900 uppercase tracking-widest mb-4">Informasi Keberangkatan</h3>
                            <div class="space-y-3">
                                <div>
                                    <p class="text-xs text-slate-500 font-medium">Tanggal Berangkat</p>
                                    <p class="font-bold text-slate-900">{{ \Carbon\Carbon::parse($booking->travel_date)->format('d F Y') }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-slate-500 font-medium">Lokasi Jemput</p>
                                    <p class="font-bold text-slate-900">{{ $booking->pickup_address }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-slate-500 font-medium">Lokasi Tujuan</p>
                                    <p class="font-bold text-slate-900">{{ $booking->dropoff_address }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-slate-500 font-medium">Jumlah Penumpang</p>
                                    <p class="font-bold text-slate-900">{{ $booking->quantity }} Orang (Kursi: {{ implode(', ', $booking->seat_numbers ?? []) }})</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                                <div class="flex justify-between mb-3 text-slate-600">
                                    <span>Harga Dasar (x{{ $booking->quantity }})</span>
                                    <span>Rp {{ number_format($booking->route->base_price * $booking->quantity, 0, ',', '.') }}</span>
                                </div>
                                <div class="flex justify-between mb-3 text-slate-600">
                                    <span>Upgrade Makan</span>
                                    <span>{{ $booking->meal_upgrade ? 'Ya (+ Rp 50.000)' : 'Tidak' }}</span>
                                </div>
                    ...
                    <div class="pt-8 border-t border-slate-100" x-data="{
                        ...
                        <div class="flex flex-col sm:flex-row gap-4 items-center justify-between">
                            ...
                            @if(in_array($booking->payment_status, ['unpaid', 'pending']))
                                ...
                                <!-- Manual Form -->
                                <form id="manual-payment-form" x-show="method !== 'midtrans'" style="display: none;" action="{{ route('shuttle.manual', $booking->booking_code) }}" method="POST" enctype="multipart/form-data" class="w-full">
                                    @csrf
    ...
    @push('scripts')
    @if(in_array($booking->payment_status, ['unpaid', 'pending']) && $booking->snap_token)
