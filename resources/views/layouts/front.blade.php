                ...
                <div class="hidden md:flex items-center space-x-3 lg:space-x-4">
                    @auth
                        ...
                        <div class="relative" x-data="{ notifOpen: false }">
                            ...
                            <div x-show="notifOpen" @click.away="notifOpen = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95 translate-y-2" x-transition:enter-end="opacity-100 scale-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100 translate-y-0" x-transition:leave-end="opacity-0 scale-95 translate-y-2" class="absolute right-0 w-80 mt-3 bg-white border border-slate-100 rounded-2xl shadow-xl z-50 overflow-hidden" style="display: none;">
                                <div class="px-4 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                    ...
                                    <form action="{{ route('notifications.read-all') }}" method="POST">
                                        @csrf
                                        <button type="submit" class="text-xs font-semibold text-sky-600 hover:text-sky-800 transition-colors">Tandai semua dibaca</button>
                                    </form>
                                    @endif
                                </div>
                                <div class="max-h-80 overflow-y-auto">
                                    @forelse(auth()->user()->notifications as $notification)
                                        <a href="{{ $notification->data['url'] ?? '#' }}" class="block px-4 py-3 border-b border-slate-50 transition-colors {{ $notification->unread() ? 'bg-sky-50/50 hover:bg-sky-100/50' : 'hover:bg-slate-50' }}">
                                            <div class="flex items-start gap-3">
                                                <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 mt-0.5 {{ $notification->data['type'] === 'success' ? 'bg-emerald-100 text-emerald-600' : ($notification->data['type'] === 'danger' ? 'bg-rose-100 text-rose-600' : 'bg-sky-100 text-sky-600') }}">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                </div>
                                                <div>
                                                    <p class="text-sm font-bold {{ $notification->unread() ? 'text-slate-900' : 'text-slate-700' }}">{{ $notification->data['title'] ?? 'Info' }}</p>
                                                    <p class="text-xs mt-1 leading-relaxed {{ $notification->unread() ? 'text-slate-600' : 'text-slate-500' }}">{{ $notification->data['message'] ?? '' }}</p>
                                    ...
                                    @empty
                                        <div class="px-4 py-8 text-center">
                                            ...
                                            <p class="text-sm font-medium text-slate-500">Belum ada notifikasi baru</p>
                                        </div>
                                    @endforelse
                                </div>
                            </div>
